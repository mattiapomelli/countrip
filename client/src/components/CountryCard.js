  ...
  useEffect(() => {
    ...
    properties.forEach((property) => {
      ...
      const countryProperty =
        property === 'gini'
          ? selected.gini
            ? Object.values(selected.gini).slice(-1)[0]
            : 0
          : selected[property]

      if (countryProperty > average[property]) {
        selectedDiv.style.width = '100%'
        averageDiv.style.width = `${
          (average[property] * 100) / countryProperty
        }%`
      } else {
        averageDiv.style.width = '100%'
        selectedDiv.style.width = `${
          (countryProperty * 100) / average[property]
        }%`
      }
    })
  }, [selected, average])
    ...
    <>
      <div className="card-header flex-container">
        <div className="header-left flex-container">
          <img src={selected.flags.svg} alt="country flag" />
        </div>
        <div className="header-right flex-container">
          <div className="main-info">
            <div className="country-name">{selected.name.common}</div>
      ...
      <div className="card-body flex-container">
        ...
        <div id="slider">
              ...
              <div className="slide slide_1">
                <div className="slide-content">
                  <div className="card-legend flex-container">
                    <i style={{ backgroundColor: '#c4c4c4' }}></i>
                    <span>Average</span>
                    <i style={{ backgroundColor: '#6671DA' }}></i>
                  ...
                  <div className="data-row flex-container">
                    ...
                    <div className="data-item column-2">
                      <h5>Area (km2)</h5>
                      <h1>
                        {selected.area ? formatNumber(selected.area) : 'N.A.'}
                      </h1>
                  ...
                  <div className="data-row flex-container">
                    ...
                    <div className="data-item column-2">
                      <span className="span-title">Gini</span>
                      <Tooltip />
                      <h1>
                        {selected.gini
                          ? Object.values(selected.gini).slice(-1)[0]
                ...
                <div className="slide-content">
                  <div className="data-row flex-container">
                    <div className="data-item column-1">
                      <h5>Languages</h5>
                      <h4>
                        {Object.values(selected.languages).map(
                          (language, index) => {
                            return (
                              <span key={index}>
                                {language}
                                {index ===
                                Object.values(selected.languages).length - 1
                                  ? ''
                                  : ', '}
                              </span>
                            )
                    ...
                    <div className="data-item column-2">
                      <h5>Currencies</h5>
                      <h4>
                        {Object.values(selected.currencies).map(
                          (currency, index) => {
                            return (
                              <span key={index}>
                                {' '}
                                {currency.name}
                                {index ===
                                Object.values(selected.currencies).length - 1
                                  ? ''
                                  : ', '}
                              </span>
                            )
                  ...
                  <div className="data-row flex-container">
                    <div className="data-item column-1">
                      <h5>Timezones (UTC)</h5>
                      <h4>
                        {selected.timezones.map((timezone, index) => {
                          return (
                            <span key={index}>
                              {' '}
                              {timezone.substring(3)}
                              {index === selected.timezones.length - 1
                                ? ''
                                : ', '}
                            </span>
                          )
                    ...
                    <div className="data-item column-2">
                      ...
                      <h4>
                        {selected.regionalBlocs.map((block, index) => {
                          return (
                            <span key={index}>
                              {block.name}
                              {index === selected.regionalBlocs.length - 1
                                ? ''
                                : ', '}
                            </span>
                          )
                ...
                <div className="slide-content">
                    ...
                    <div className="data-item column-1">
                      ...
                      <h3>
                        {selected.name.nativeName &&
                          Object.values(selected.name.nativeName)[0].common}
                      </h3>
                    </div>
                    <div className="data-item column-2">
                      ...
                      <h3>
                        {' '}
                        {selected.tld.map((domain, index) => {
                          return (
                            <span key={index}>
                              {' '}
                              {domain}
                              {index === selected.tld.length - 1 ? '' : ', '}
                            </span>
                          )
                        })}
                      </h3>
                  ...
                  <div className="data-row flex-container">
                    <div className="data-item column-1">
                      {/* <h5>Calling Codes</h5>
                      <h3>
                        {selected.callingCodes.map((code, index) => {
                          return (
                            <span key={index}>
                              +{code}
                              {index === selected.callingCodes.length - 1
                                ? ''
                                : ', '}
                            </span>
                          )
          ...
          <div className="bullets flex-container">
            <i
              className={`${slide === 1 ? 'bullet-active' : ''}`}
              onClick={() => setSlide(1)}
            ></i>
            <i
              className={`${slide === 2 ? 'bullet-active' : ''}`}
              onClick={() => setSlide(2)}
            ></i>
            <i
              className={`${slide === 3 ? 'bullet-active' : ''}`}
              onClick={() => setSlide(3)}
            ></i>
          </div>
        </div>
