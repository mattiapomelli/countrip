...
export default ({ children }) => {
  ...
  const getCountryData = (code) => {
    //get data for a specific country when selected, to display in the country card
    axios
      .get(`https://restcountries.com/v3.1/alpha/${code}`)
      .then((res) => {
        const country = res.data[0]

        let density = country.area
          ? Math.ceil(country.population / country.area)
          : null

        setSelected({ ...country, density })
      })
      .catch((err) => console.log(err))
  }

  const getAllCountriesData = () => {
    //get basic data for all countries to display in the table
    axios
      .get('https://restcountries.com/v3.1/all')
      .then((res) => {
        /* keep only official countries, and discard non relevant ones */
        const result = res.data.filter((item) =>
          countryCodes.includes(item.cca3)
        )
        ...
        for (let i = 0; i < result.length; i++) {
          ...
          sum.density += result[i].area
            ? result[i].population / result[i].area
            : 0

          if (result[i].gini) {
            const gini = Object.values(result[i].gini).slice(-1)[0]
            console.log('Gini: ', gini, result[i].cca3)
  ...
  const findLayerByCode = useCallback((code) => {
    const layers = layersRef.current.leafletElement.getLayers()
    const result = layers.find(
      (layer) => layer.feature.properties.ISO_A3 === code
    )
    return result
  }, [])
  ...
  const setActiveLayer = useCallback(
    (layer) => {
      //set active layer on the map and give it the proper style
      if (!layer.active) {
        //in this way we don't re-fetch data and change everything if the active country is clicked
        resetActiveLayer()
  ...
  const findCountryByCode = (code) => {
    const result = countries.find((country) => country.cca3 === code)
    return result
  }

  const sortCountries = (property, ascending) => {
    //receives the property to use for the sorting
    ...
    if (activeProperty === property) {
      ...
      const sorted = [...countries].sort(function (a, b) {
        //need to make a copy of the array before to sort it, otherwise it won't be detected any change in the state and the component won't re-render
